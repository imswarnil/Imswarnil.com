        ...
        if (jsPath.includes('*')) {
            ...
            if (isMainJs) {
                config.plugins.push(
                    ...
                    // Additional CSS (fixed: use relative file names, no absolute paths)
                    ...additionalCss.map((file) => {
                        // "assets/css/section-featured/slider.css"
                        //    -> "section-featured/slider.css"
                        const cssOutputName = file.replace(/^assets\/css\//, '');

                        return postcss({
                            include: file,
                            extract: cssOutputName, // stays inside output.dir (assets/built)
                            minimize: isProduction,
                            plugins: cssPlugins,
                        });
                    }),
